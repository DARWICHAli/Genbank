...
from terminate_thread import TerminateThread
...
class Genbank(QtWidgets.QMainWindow, QtCore.QObject):
	...
	terminate_parsing_signal =  QtCore.pyqtSignal()
	...
	def worker(self):
		...
		if( self.isRunning == False):
			...
			if not len(self.region_choice):
				...
				self.log("Erreur User: Il faut choisir au moins une région fonctionnelle!", red)
			...
			self.thread[2].fin_parsing_signal.connect(self.activate_button)
			self.thread[4] = TerminateThread(parent = self, thread = self.thread[2])
			self.thread[2].start()
			self.thread[4].start()
			...
			self.mainwindow.buttonStart.setText("Waiting for threads...")
			self.mainwindow.buttonStart.setStyleSheet("background-color: rgb(180, 150, 150);\n" "color:rgb(0, 4, 38);")
