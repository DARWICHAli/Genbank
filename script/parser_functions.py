    ...
    for NC in NC_LIST:
        ...
        for f in features:
            if f.type in selected_regions:
                ...
                if f.location:
                    if(manage_errors(f, len(handle_read.seq), log_signal)):
                        continue
                    ...
                    if f.location.strand == -1:  # complement detection
                        ...
                        if f.location_operator == "join":  # complement(join) detection
                            if f.type == "CDS":
                                if intron_is_selected:
                                    try:
                                        [intron_seq, nb] = join(header, handle_read.seq, f.location, log_signal, True)
                                        nb_introns += nb
                                    # gestion erreur
                                    except:
                                        continue
                                ...
                                if cds_is_selected:
                                    cds_seq = join(header, handle_read.seq, f.location, log_signal)[0]
                                    # gestion erreur
                                    if not cds_seq:
                                        continue
                            else:
                                final_seq = join(header, handle_read.seq, f.location, log_signal)[0]
                                # gestion erreur
                                if not final_seq:
                                    continue
...
def join(header, sequence, location, log_signal, intron=False):
    final_seq = ""
